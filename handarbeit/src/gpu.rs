...
// INFO: See comments in th old/handarbeit/ folder, where all of this is explained.
impl GpuState {
    pub async fn new(window: Arc<Window>) -> Result<Self, String> {
        ...
        // This is the default render pipeleine, so the shaders etc. compiled and passed intop the GPU.
    ...
    // TODO: we need to reuse buffers all around. The rendering path is way too performance critical
    // toa allocate new buffers every time. Also see the drawing list which also needs to be
...

        {
            let mut pass = encoder.begin_render_pass(&wgpu::RenderPassDescriptor {
                ...
                color_attachments: &[Some(wgpu::RenderPassColorAttachment {
                    ...
                    resolve_target: None,
                    ops: wgpu::Operations {
                        // Clearing the buffer/screen with a particular color, and using that in the
                        // output.
            ...
            // Here the GPU actually draws.
            pass.draw(0..vertices.len() as u32, 0..1);
        }

        self.queue.submit([encoder.finish()]);
        self.window.pre_present_notify();
        // Here we swap buffers and show our drawing.
        output.present();
...
// Here we convert all rectangles to triangles the GPU can render.
...
impl Vertex {
    ...
    // Here we specify the input of the vertex shader so we can use the Vertex type above inside the
    // shader. This will be passed to the GPU.
...
// This is the most simple shader there is. We basically do nothing and just ouput was was input. We
// need to change a vec2 input to a vec4 output since the GPU expects that (z-axis, some scaling
// number for x,y,z).
const SHADER: &str = r#"
struct VsOut {
